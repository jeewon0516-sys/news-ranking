...
def is_major_korean(a: dict) -> bool:
    """주요 국내 언론사 기사인지 판별."""
    # 직접 등록된 RSS 소스는 무조건 통과
    if a["source"] in STATIC_SOURCES:
        return True
...
def get_pr_google_news_url(keyword: str) -> str:
    """카테고리 모드: '보도자료' 추가 검색으로 PR 콘텐츠 우선."""
    enc = urllib.parse.quote(f"{keyword} 보도자료")
    return f"https://news.google.com/rss/search?q={enc}&hl=ko&gl=KR&ceid=KR:ko"
...
def fetch_pr_keyword(keyword: str, per_kw: int = 5) -> List[Dict]:
    ...
    - 중복 제거 후 per_kw개 반환
    """
    tasks: Dict[str, str] = {"Google News (PR)": get_pr_google_news_url(keyword)}
    ...
    # pr_score 높은 순 → 최신순
    all_arts.sort(key=lambda a: (a.get("pr_score", 0), a["rec"]), reverse=True)
